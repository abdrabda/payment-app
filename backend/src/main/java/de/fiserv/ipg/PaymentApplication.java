...
public class PaymentApplication {
  ...
  CommandLineRunner initDatabase(TransactionRepository repository) {
    return args -> {
      ...
      for (int i = 1; i <= 20; i++) {
        Transaction.TransactionType transactionType = (i % 2 == 0) ? Transaction.TransactionType.SALE
                : Transaction.TransactionType.REFUND;
        Transaction.Status status = Transaction.Status.values()[rand.nextInt(Transaction.Status.values().length)];
        Transaction.Currency currency = Transaction.Currency.values()[rand
                .nextInt(Transaction.Currency.values().length)];
